...
export const RazorpayPaymentButton = ({
    ...
    const handlePayment = useCallback(async () => {
        ...
        const options: RazorpayOrderOptions = {
            callback_url: `${process.env.MEDUSA_BACKEND_URL}/razorpay/hooks`,
            key: process.env.NEXT_PUBLIC_RAZORPAY_KEY ?? "",
            amount: session.amount * 100 * 100,
            order_id: orderData.id,
            currency: cart.currency_code.toUpperCase() as CurrencyCode,
            name: process.env.NEXT_PUBLIC_SHOP_NAME ?? "Yadhronics ",
            description: `Order number ${orderData.id}`,
            remember_customer: true,

            image: "https://d19u8jta4am29c.cloudfront.net/yadhronics_logo.jpg",
            modal: {
                backdropclose: true,
                escape: true,
                handleback: true,
